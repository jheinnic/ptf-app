...
class LoadDensityModel:
    ...
    def ensure_lanes(self):
        if (self.lane_images is not None):
            return
        if path.exists(self.lane_list):
            with open(self.lane_list, "r") as lane_reader:
                self.lane_images = [
                    line.split("::")[0] for line in lane_reader.readlines()
                ]
            return
        with open(self.lane_list, "w") as lane_writer:
    ...
    def ensure_feature_vectors(self):
        ...
        for feature_index in range(0, len(self.feature_files)):
            ...
            if path.exists(feature_file):
                ...
                self.ensure_lanes()
                with open(feature_file, "w") as fileOut:
                    vector_data = []
                    writer = csv.writer(
                        fileOut, delimiter="|", lineterminator="\n", strict=1
                    )
                    print(self.lane_images)
