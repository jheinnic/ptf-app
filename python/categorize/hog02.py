...
img = imread(img)
img2 = np.array([img[:,:,0],img[:,:,2],img[:,:,1]])
img2 = img2.reshape([img.shape[0]*3, img.shape[1]])
print(img.shape)
print(img2.shape)
...
rescale_shape = (
    rescale_base * (np.array(img2.shape) / rescale_base).round()
).astype('uint16').tolist()
print(rescale_shape)

resized_img = resize(img2, rescale_shape)
...
plt.imsave("hog02mp.png", hog_image, cmap="gray")
imsave("hog02.png", (hog_image * 255 / hog_image.max()).astype('uint8'))
