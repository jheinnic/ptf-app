...
from jchptf_image_upload.signed_image_uploads_pb2_grpc import ImageUploadSigningServiceStub
from jchptf_image_upload.signed_image_uploads_pb2 import CreateSignedUploadSingleRequest, \
    OptionalUploadFeature, SignedUploadSingleReply, FlaggedOptionalFeature
...
_worker_channel_singleton: Optional[grpc.Channel] = None
_worker_stub_singleton: Optional[ImageUploadSigningServiceStub] = None
...
def _initialize_worker(server_address):
    ...
    _worker_stub_singleton = ImageUploadSigningServiceStub(_worker_channel_singleton)
...
def _run_worker_query(a, _dummy_var="missing") -> Tuple[int, bytes]:
    print(f"Calling API on {a}, and {_dummy_var}?")
...
def _get_one_uuid(server_address) -> SignedUploadSingleReply:
    ...
    result = worker_pool.apply(_run_worker_query, args=("localhost:52432",))
