...
def _register_plot_points(tx: Transaction, pixel_height: int, pixel_width: int,
                          window_y: float, window_x: float, window_width: float,
                          window_point: WindowPoint, pixel_unit: int = 1):
    (bottom, top, left, right) = check_proportions(
        window_y, window_x, window_width, window_point)
    (height_dim, width_dim) = compute_dimensions(
        pixel_height, bottom, top, pixel_width, left, right, pixel_unit=pixel_unit)

    # TODO: Commit dimension lists to an object store using plot_hash for key
    ...
    tx.run("MERGE (r:Resolution {height: $height, width: $width}) "
           ...
           plot_hash=plot_hash, density=(pixel_height / (top - bottom)))
